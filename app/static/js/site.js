...
document.addEventListener('click', event => {
    ...
    section.querySelectorAll('[data-filter]').forEach(other => { other.classList.toggle('active', other === button); other.setAttribute('aria-pressed', String(other === button)); });
...
if (select && service && Array.from(select.options).some(option => option.value === service))
...
form?.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(typeof result.detail === 'string' ? result.detail : 'Bitte prüfe deine Eingaben.');
