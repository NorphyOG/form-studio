...
export function getCatalog() {
    if (!cache)
        cache = api('/api/admin/catalog').then(catalog => {
...
export function miniModule(type) { return `<div class="module-mini mini-${e(type)}" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></div>`; }
export async function openCatalog(catalog, user, remaining, onInsert) {
    const presets = await api('/api/admin/block-presets');
    ...
    const input = dialog.querySelector('#library-search');
    function insert(blocks) { if (blocks.length > remaining) {
        toast(`Es sind nur noch ${remaining} Modulplätze frei.`, true);
        return;
    } onInsert(blocks.map(cloneBlock)); dialog.close(); }
    function render() {
        ...
        const cats = dialog.querySelector('.library-categories');
        ...
        cats.querySelectorAll('button').forEach(b => b.addEventListener('click', () => { category = b.dataset.category; render(); }));
        if (tab === 'modules') {
            ...
            content.querySelectorAll('[data-insert]').forEach(b => b.addEventListener('click', () => insert([catalog.modules.find(m => m.type === b.dataset.insert).defaults])));
        }
            ...
            content.querySelectorAll('[data-template]').forEach(b => b.addEventListener('click', () => insert(catalog.templates.find(t => t.id === b.dataset.template).blocks)));
        }
        else {
            ...
            content.querySelectorAll('[data-preset]').forEach(b => b.addEventListener('click', () => insert([presets.find(p => p.id === b.dataset.preset).block])));
            content.querySelectorAll('[data-delete-preset]').forEach(b => b.addEventListener('click', async () => { if (!confirm('Diese Vorlage entfernen? Bereits eingefügte Kopien bleiben erhalten.'))
                return; try {
                await api('/api/admin/block-presets/' + b.dataset.deletePreset, { method: 'DELETE' });
                presets.splice(presets.findIndex(p => p.id === b.dataset.deletePreset), 1);
                render();
    ...
    dialog.querySelectorAll('[data-tab]').forEach(b => b.addEventListener('click', () => { tab = b.dataset.tab; render(); }));
    input.addEventListener('input', render);
    render();
    input.focus();
}
export function saveAsPreset(block) {
    ...
    dialog.querySelector('form').addEventListener('submit', async (event) => { event.preventDefault(); const button = dialog.querySelector('button[type=submit]'); button.disabled = true; try {
        await api('/api/admin/block-presets', json('POST', { name: (new FormData(event.target)).get('name'), block }));
        dialog.close();
        toast('Baustein in der gemeinsamen Bibliothek gespeichert.');
    }
    catch (err) {
        dialog.querySelector('.form-error').textContent = errorMessage(err);
    }
    finally {
