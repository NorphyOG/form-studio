...
export class PageBuilder {
    ...
    timer;
    editable;
    constructor(root, initial, catalog, media, user, onChange) {
        this.root = root;
        ...
        this.snapshots = [JSON.stringify(this.blocks)];
        this.render();
    }
    get value() { this.collect(); return structuredClone(this.blocks); }
    destroy() { clearTimeout(this.timer); }
    collect() {
        this.root.querySelectorAll('[data-block-index]').forEach(row => {
            const block = this.blocks[Number(row.dataset.blockIndex)];
            if (!block)
                return;
            const next = block;
            row.querySelectorAll('[data-bfield]').forEach(input => { next[input.dataset.bfield] = input instanceof HTMLInputElement && input.type === 'checkbox' ? input.checked : ['columns', 'limit'].includes(input.dataset.bfield) ? Number(input.value) : input.value; });
            const itemRows = row.querySelectorAll('[data-item-index]');
            if (itemRows.length || this.catalog.modules.find(m => m.type === block.type).supports.includes('items')) {
                block.items = Array.from(itemRows).map(itemRow => {
                    ...
                    const item = { ...old };
                    itemRow.querySelectorAll('[data-ifield]').forEach(input => { item[input.dataset.ifield] = input.value; });
                    return item;
                });
            }
        });
    ...
    remember() {
        ...
        if (state === this.snapshots[this.cursor])
            return;
    ...
    syncHistoryButtons() {
        const undo = this.root.querySelector('[data-undo]'), redo = this.root.querySelector('[data-redo]');
        if (undo)
            undo.disabled = !this.editable || this.cursor === 0;
        if (redo)
            redo.disabled = !this.editable || this.cursor === this.snapshots.length - 1;
    }
    change(callback) {
        if (!this.editable)
            return;
    ...
    insert(blocks) {
        if (this.blocks.length + blocks.length > this.catalog.limits.blocks) {
            toast('Eine Seite kann höchstens 30 Module enthalten.', true);
            return;
        }
        this.change(() => { this.selected = this.blocks.length; this.blocks.push(...blocks); });
        this.focusIndex(this.selected);
    }
    focusId(id) { const index = this.blocks.findIndex(b => b.id === id); if (index >= 0)
        this.focusIndex(index); }
    focusIndex(index) {
        this.selected = index;
        this.root.dispatchEvent(new CustomEvent('studio-module-selected', { bubbles: true, detail: { id: this.blocks[index]?.id } }));
        this.root.querySelectorAll('.block-editor').forEach((row, i) => { row.open = i === index; });
        const row = this.root.querySelector(`[data-block-index="${index}"]`);
        row?.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
        this.root.querySelectorAll('[data-outline]').forEach(b => b.classList.toggle('active', Number(b.dataset.outline) === index));
    }
    blockFields(block, index) {
        const spec = this.catalog.modules.find(m => m.type === block.type);
        const supports = spec.supports;
        const maxItems = ['contact', 'comparison'].includes(block.type) ? 2 : 20;
        const opts = this.catalog.assets.map(a => [a.id, a.name]);
        const images = [['', 'Standardgrafik verwenden'], ...this.media.map(m => [m.id, m.alt])];
        const f = (label, key, value, helpKey = key, extra = '') => field(label, `b${index}-${key}`, value, 'text', `data-bfield="${key}" ${extra}`, helpKey);
        const t = (label, key, value, helpKey = key) => textarea(label, `b${index}-${key}`, value, key === 'text' ? 3 : 2, `data-bfield="${key}" maxlength="${key === 'text' ? 10000 : 150}"`, helpKey);
        const s = (label, key, value, options) => select(label, `b${index}-${key}`, value, options, `data-bfield="${key}"`, key);
   ...
   ${supports.includes('news') ? `<div class="field-grid">${f('Kategorie (leer = alle)', 'category', block.category || '', 'news_category', 'maxlength="50"')}${s('Anzahl Beiträge', 'limit', String(block.limit || 6), Array.from({ length: 12 }, (_, i) => [String(i + 1), String(i + 1)]))}</div><p class="field-hint">Beiträge unter „Journal & News“ anlegen und veröffentlichen. Der Inhalt bleibt zentral gepflegt.</p>` : ''}
   ...
   ${supports.includes('items') ? `<div class="items-heading"><h4>Einträge <span>${block.items.length} / ${maxItems}</span></h4>${helpButton('items')}</div><div class="block-items">${block.items.map((item, j) => {
            const attr = (key) => `data-ifield="${key}"`;
            const name = (key) => `b${index}-i${j}-${key}`;
   ...
   <details class="appearance-section"><summary>Darstellung & Verhalten <span>Farbe · Abstand · Animation</span></summary><div class="field-grid">${s('Abschnittsfarbe', 'theme', block.theme || 'paper', [['paper', 'Hell / Standard'], ['blue', 'Akzentfarbe'], ['dark', 'Dunkel']])}${s('Abstand davor', 'spacing', block.spacing || 'normal', [['compact', 'Kompakt'], ['normal', 'Normal'], ['spacious', 'Großzügig']])}${s('Breite', 'width', block.width || 'wide', [['wide', 'Breit'], ['narrow', 'Schmal']])}${s('Animation', 'animation', block.animation || 'auto', [['auto', 'Automatisch'], ['reveal', 'Sanft einblenden'], ['slide', 'Seitlich einfahren'], ['unfold', 'Auffalten / Bühne'], ['stack', 'Gestaffelte Karten'], ['repeat', 'Bei Rückkehr erneut'], ['none', 'Ohne Bewegung']])}${s('Überschrift ausrichten', 'align', block.align || 'left', [['left', 'Linksbündig'], ['center', 'Zentriert']])}${supports.includes('columns') ? s('Spalten auf Desktop', 'columns', String(block.columns || 3), [['2', 'Zwei'], ['3', 'Drei'], ['4', 'Vier']]) : ''}</div>${f('Abschnittsanker', 'id', block.id, 'anchor', 'required pattern="[a-zA-Z0-9_-]{1,50}" maxlength="50"')}</details><div class="module-preset-row"><button type="button" class="button small" data-save-preset="${index}">⧉ Als Baustein speichern</button>${helpButton('preset')}</div>`;
    ...
    render() {
        ...
        if (!this.editable)
            this.root.querySelectorAll('input,textarea,select,button:not(.help-trigger):not([data-outline])').forEach(el => el.disabled = true);
        this.syncHistoryButtons();
        this.root.querySelectorAll('[data-outline]').forEach(b => b.addEventListener('click', () => this.focusIndex(Number(b.dataset.outline))));
        this.root.querySelectorAll('[data-open-library]').forEach(b => b.addEventListener('click', () => { this.collect(); void openCatalog(this.catalog, this.user, 30 - this.blocks.length, blocks => this.insert(blocks)).catch(err => toast(errorMessage(err), true)); }));
        this.root.querySelector('[data-undo]')?.addEventListener('click', () => { this.collect(); this.remember(); if (this.cursor > 0) {
            this.blocks = JSON.parse(this.snapshots[--this.cursor]);
            this.render();
            this.onChange();
        } });
        this.root.querySelector('[data-redo]')?.addEventListener('click', () => { if (this.cursor < this.snapshots.length - 1) {
            this.blocks = JSON.parse(this.snapshots[++this.cursor]);
            this.render();
            this.onChange();
        } });
        this.root.querySelectorAll('[data-move]').forEach(b => b.addEventListener('click', () => this.change(() => { const [i, d] = b.dataset.move.split(':').map(Number), j = i + d; if (j >= 0 && j < this.blocks.length) {
            [this.blocks[i], this.blocks[j]] = [this.blocks[j], this.blocks[i]];
            this.selected = j;
        } })));
        this.root.querySelectorAll('[data-duplicate-block]').forEach(b => b.addEventListener('click', () => { if (this.blocks.length >= 30)
            return; this.change(() => { const i = Number(b.dataset.duplicateBlock); this.blocks.splice(i + 1, 0, cloneBlock(this.blocks[i])); this.selected = i + 1; }); }));
        this.root.querySelectorAll('[data-remove-block]').forEach(b => b.addEventListener('click', () => { if (confirm('Dieses Modul aus dem Entwurf entfernen? Mit Rückgängig kannst du es in dieser Sitzung wiederherstellen.'))
            this.change(() => this.blocks.splice(Number(b.dataset.removeBlock), 1)); }));
        this.root.querySelectorAll('[data-save-preset]').forEach(b => b.addEventListener('click', () => { this.collect(); saveAsPreset(this.blocks[Number(b.dataset.savePreset)]); }));
        this.root.querySelectorAll('[data-add-item]').forEach(b => b.addEventListener('click', () => this.change(() => { const block = this.blocks[Number(b.dataset.addItem)]; if (block.items.length < (['contact', 'comparison'].includes(block.type) ? 2 : 20))
            block.items.push({ title: 'Neuer Eintrag', text: '', art: 'brand', value: '', link_label: '', link_url: '', image: '' }); })));
        this.root.querySelectorAll('[data-remove-item]').forEach(b => b.addEventListener('click', () => this.change(() => { const [i, j] = b.dataset.removeItem.split(':').map(Number); this.blocks[i].items.splice(j, 1); })));
        this.root.querySelectorAll('[data-item-move]').forEach(b => b.addEventListener('click', () => this.change(() => { const [i, j, d] = b.dataset.itemMove.split(':').map(Number), items = this.blocks[i].items; if (j + d >= 0 && j + d < items.length)
            [items[j], items[j + d]] = [items[j + d], items[j]]; })));
        this.root.querySelectorAll('input,textarea,select').forEach(input => input.addEventListener('input', () => {
            this.collect();
            clearTimeout(this.timer);
            this.timer = window.setTimeout(() => this.remember(), 650);
            this.onChange();
            const row = input.closest('[data-block-index]');
            if (row) {
                const block = this.blocks[Number(row.dataset.blockIndex)];
                row.classList.toggle('is-disabled', !block.enabled);
                row.querySelector('summary small').textContent = block.title.replace(/\n/g, ' ') || 'Ohne Titel';
                const outline = this.root.querySelector(`[data-outline="${row.dataset.blockIndex}"]`);
                if (outline) {
                    outline.querySelector('small').textContent = block.title.replace(/\n/g, ' ').slice(0, 38) || 'Ohne Titel';
                    outline.classList.toggle('outline-disabled', !block.enabled);
                    outline.querySelector('i').textContent = block.enabled ? '' : '○';
                }
            }
        ...
        this.root.querySelectorAll('[data-pick-media],[data-pick-item-media]').forEach(button => button.addEventListener('click', () => {
            const locator = button.dataset.pickItemMedia;
            const index = Number(locator ? locator.split(':')[0] : button.dataset.pickMedia);
            const row = this.root.querySelector(`[data-block-index="${index}"]`);
            const target = locator ? row.querySelector(`[data-item-index="${locator.split(':')[1]}"] select[data-ifield="image"]`) : row.querySelector('select[data-bfield="image"]');
            if (target)
                void pickMedia(media => { if (media && !this.media.some(m => m.id === media.id))
                    this.media.push(media); setMediaSelect(target, media); });
        }));
        this.root.querySelectorAll('[data-pick-campaign]').forEach(button => button.addEventListener('click', () => {
            const index = Number(button.dataset.pickCampaign);
            void pickCampaign(item => this.change(() => { this.blocks[index].campaign = item?.id || ''; }));
        }));
        // Drag only from an explicit handle; arrow controls provide a touch/keyboard alternative.
        let source = -1;
        this.root.querySelectorAll('[data-block-index]').forEach(row => {
            row.addEventListener('dragstart', event => { if (!this.editable || !event.target.closest('.drag-handle')) {
                event.preventDefault();
                return;
            } source = Number(row.dataset.blockIndex); event.dataTransfer?.setData('text/plain', String(source)); row.classList.add('dragging'); });
            row.addEventListener('dragover', event => { if (source >= 0) {
                event.preventDefault();
                row.classList.add('drop-target');
            } });
            row.addEventListener('dragleave', () => row.classList.remove('drop-target'));
            row.addEventListener('drop', event => { event.preventDefault(); const target = Number(row.dataset.blockIndex); if (source >= 0 && source !== target)
                this.change(() => { const [moved] = this.blocks.splice(source, 1); this.blocks.splice(target, 0, moved); this.selected = target; }); source = -1; row.classList.remove('drop-target'); });
