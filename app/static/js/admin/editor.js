...
function defaultData(kind) {
    ...
    if (kind === 'campaign')
        return { ...base, sponsor: '', body: '', image: '', art: 'rings', link_label: 'Mehr erfahren', link_url: '/kontakt', starts_on: '', ends_on: '' };
    if (kind === 'article')
        return { ...base, art: 'print', body: '', tags: [], image: '', category: 'Studio', author: 'Redaktion', published_on: new Date().toISOString().slice(0, 10), featured: false, blocks: [] };
...
export async function openEditor(record, kind, user, refresh, initial) {
    let current = record;
    const data = { ...structuredClone(record?.data || defaultData(kind)), ...initial };
    if (kind === 'article' && !record)
        data.author = user.name;
    const hasBlocks = kind === 'page' || kind === 'article';
    const [media, catalog] = await Promise.all([api('/api/admin/media'), getCatalog()]);
    ...
    const walk = (value) => { if (Array.isArray(value))
        ...
        Object.values(o).forEach(v => { if (typeof v === 'object')
            walk(v); });
    } };
    walk(data);
    await Promise.all(Array.from(referenced).filter(id => !media.some(m => m.id === id)).map(async (id) => { try {
        media.push(await api('/api/admin/media/' + id));
    }
    catch { /* Missing references use the standard illustration. */ } }));
    const canEdit = user.role !== 'moderator';
    const assetOptions = catalog.assets.map(a => [a.id, a.name]);
    const mediaOptions = [['', 'Standardgrafik verwenden'], ...media.map(m => [m.id, m.alt])];
 ...
 <footer class="editor-footer"><div class="save-indicator"><span class="save-dot"></span><span id="save-status" role="status">${canEdit ? 'Vorschau speichert nicht. Entwurf bewusst sichern.' : 'Prüfansicht · keine Bearbeitungsrechte'}</span></div><div><a class="button small" target="_blank" rel="noopener" id="live-link" hidden>Live ↗</a>${canEdit ? `<span class="action-help"><button type="submit" form="record-form" class="button" id="save-button">Entwurf speichern</button>${helpButton('save')}</span><span class="action-help"><button type="button" class="button" id="submit-button">Zur Prüfung</button>${helpButton('submit')}</span>` : ''}${user.role === 'admin' ? '<button type="button" class="button primary" id="publish-button">Veröffentlichen ↗</button>' : ''}${user.role === 'moderator' ? '<button type="button" class="button primary" id="approve-button" hidden>Freigeben ↗</button>' : ''}<button type="button" class="button small danger-text" id="unpublish-button" hidden>Offline nehmen</button></div></footer></div>`);
    dialog.className = 'editor-dialog editor-v2' + (kind !== 'settings' ? ' with-preview' : '');
    ...
    let dirty = !!initial, busy = false, closed = false, previewTimer, previewSequence = 0, previewAbort;
    let builder;
    function markDirty() { dirty = true; dialog.querySelector('#save-status').textContent = 'Ungespeichert · Strg / Cmd + S zum Sichern'; dialog.querySelector('.save-dot').classList.add('is-dirty'); schedulePreview(); }
    if (hasBlocks)
        builder = new PageBuilder(dialog.querySelector('#page-builder'), data.blocks || [], catalog, media, user, markDirty);
    if (!canEdit)
        form.querySelectorAll('.meta-fields input,.meta-fields textarea,.meta-fields select').forEach(el => el.disabled = true);
    form.addEventListener('input', event => { if (!event.target.closest('#page-builder'))
        markDirty(); });
    const designSelect = form.querySelector('[name=design]');
    const syncDesign = () => { dialog.querySelectorAll('[data-design-option]').forEach(button => button.setAttribute('aria-pressed', String(button.dataset.designOption === designSelect?.value))); };
    ...
    form.querySelector('[name=title]').addEventListener('input', () => {
        if (!current && !slug.dataset.edited)
            slug.value = val('title').toLowerCase().replace(/ä/g, 'ae').replace(/ö/g, 'oe').replace(/ü/g, 'ue').replace(/ß/g, 'ss').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 64);
    ...
    function collect() {
        const next = { ...data, title: val('title'), slug: val('slug'), description: val('description') };
        if (kind === 'page')
            Object.assign(next, { blocks: builder.value, in_navigation: checked('in_navigation'), navigation_label: val('navigation_label'), order: Number(val('order') || 0) });
        if (kind === 'service' || kind === 'project' || kind === 'article')
            Object.assign(next, { art: val('art'), body: val('body'), tags: val('tags').split(',').map(t => t.trim()).filter(Boolean), image: val('image'), order: Number(val('order') || 0) });
        if (kind === 'article')
            Object.assign(next, { category: val('category'), author: val('author'), published_on: val('published_on'), blocks: builder.value });
        if (kind === 'campaign')
            Object.assign(next, { sponsor: val('sponsor'), body: val('body'), image: val('image'), art: val('art'), link_label: val('link_label'), link_url: val('link_url'), starts_on: val('starts_on'), ends_on: val('ends_on') });
        if (kind === 'project')
            Object.assign(next, { category: val('category'), client: val('client'), year: val('year'), demo: checked('demo') });
        if (kind === 'settings')
            Object.assign(next, { brand: val('brand'), tagline: val('tagline'), accent: val('accent'), design: val('design'), email: val('email'), location: val('location'), motion: val('motion'), intro: val('intro'), analytics_enabled: checked('analytics_enabled'), news_in_navigation: checked('news_in_navigation') });
        return next;
    }
    function updateStatus() {
        ...
        dialog.querySelector('#history-button').disabled = !current;
        const live = dialog.querySelector('#live-link');
        live.hidden = !current?.live;
        if (current?.live)
            live.href = kind === 'settings' ? '/' : publicPath(current);
        live.textContent = kind === 'campaign' ? 'Kampagnenvorschau ↗' : 'Live ↗';
        dialog.querySelector('#unpublish-button').hidden = !current?.live || user.role === 'editor' || kind === 'settings';
        const approve = dialog.querySelector('#approve-button');
        if (approve)
            approve.hidden = current?.state !== 'pending';
    }
    function schedulePreview() { if (!frame || closed || previewPaused)
        return; clearTimeout(previewTimer); previewTimer = window.setTimeout(() => void renderPreview(), 500); }
    async function renderPreview() {
        if (!frame || closed || previewPaused || dialog.querySelector('.editor-preview').hidden)
            return;
        previewAbort?.abort();
        previewAbort = new AbortController();
        const sequence = ++previewSequence;
        const status = dialog.querySelector('#preview-status');
        status.textContent = 'Aktuelle Eingaben werden geprüft …';
        status.classList.remove('preview-warning');
        try {
            const next = collect();
            // A named placeholder permits an empty new page to be previewed, not saved.
            if (!next.title.trim())
                next.title = 'Neue Seite';
            if (!next.slug.trim())
                next.slug = 'vorschau';
            const result = await api('/api/admin/preview', { ...json('POST', { kind, data: next }), signal: previewAbort.signal });
            if (sequence !== previewSequence || closed || previewPaused)
                return;
            pendingHTML = result.html;
            if (frameReady)
                frame.contentWindow?.postMessage({ type: 'studio-preview-patch', html: result.html }, location.origin);
            else
                frame.srcdoc = result.html;
            status.textContent = 'Aktuelle Eingaben · nicht automatisch gespeichert. Verknüpfte Inhalte: Live-Stand.';
        }
        catch (err) {
            if (sequence !== previewSequence || closed || err instanceof DOMException && err.name === 'AbortError')
                return;
            status.textContent = 'Letzte gültige Vorschau · ' + errorMessage(err);
            status.classList.add('preview-warning');
        }
    }
    let deviceWidth = 1200;
    const stage = dialog.querySelector('.preview-stage');
    function fitPreview() { if (!frame || !stage || stage.clientWidth < 1)
        return; const scale = Math.min(1, stage.clientWidth / deviceWidth); frame.style.width = `${deviceWidth}px`; frame.style.height = `${Math.max(500, stage.clientHeight / scale)}px`; frame.style.transform = `scale(${scale})`; dialog.querySelector('#preview-size').textContent = `${deviceWidth} px · ${Math.round(scale * 100)} %`; }
    const resizeObserver = stage ? new ResizeObserver(fitPreview) : null;
    if (stage)
        resizeObserver?.observe(stage);
    dialog.querySelectorAll('[data-device]').forEach(button => button.addEventListener('click', () => { deviceWidth = Number(button.dataset.device); dialog.querySelectorAll('[data-device]').forEach(b => b.setAttribute('aria-pressed', String(b === button))); fitPreview(); }));
    dialog.querySelector('#toggle-preview')?.addEventListener('click', () => { const preview = dialog.querySelector('.editor-preview'); preview.hidden = !preview.hidden; dialog.classList.toggle('with-preview', !preview.hidden); dialog.querySelector('#toggle-preview').setAttribute('aria-pressed', String(!preview.hidden)); if (!preview.hidden) {
        fitPreview();
        void renderPreview();
    } });
    const sendPreview = (data) => frame?.contentWindow?.postMessage(data, location.origin);
    frame?.addEventListener('load', () => { if (!frame.contentDocument?.body.classList.contains('is-preview'))
        return; frameReady = true; fitPreview(); sendPreview({ type: 'studio-preview-mode', interactive: previewInteractive }); });
    ...
    dialog.querySelector('#preview-pause')?.addEventListener('click', () => { previewPaused = !previewPaused; dialog.querySelector('#preview-pause').setAttribute('aria-pressed', String(previewPaused)); dialog.querySelector('#preview-pause').textContent = previewPaused ? 'Live fortsetzen' : 'Live pausieren'; if (previewPaused) {
        ...
        void renderPreview(); });
    dialog.addEventListener('studio-module-selected', event => { const detail = event.detail; sendPreview({ type: 'studio-preview-select', id: detail.id }); });
    const onMessage = (event) => {
        if (event.origin !== location.origin || event.source !== frame?.contentWindow)
            return;
        if (event.data?.type === 'studio-select-block' && typeof event.data.id === 'string')
            builder?.focusId(event.data.id);
        if (event.data?.type === 'studio-preview-link')
            toast('Vorschaulinks öffnen keine andere Seite. Zum echten Test „Live“ verwenden.');
    };
    window.addEventListener('message', onMessage);
    const beforeUnload = (event) => { if (dirty) {
        event.preventDefault();
        event.returnValue = '';
    } };
    window.addEventListener('beforeunload', beforeUnload);
    const close = () => { if (busy) {
        toast('Der Speichervorgang läuft noch.');
        return;
    } if (!dirty || confirm('Ungespeicherte Änderungen verwerfen? Die Vorschau allein speichert nicht.')) {
        dialog.close();
        refresh();
    } };
    dialog.querySelector('.close-editor').addEventListener('click', close);
    dialog.oncancel = event => { event.preventDefault(); close(); };
    const onKey = (event) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 's' && dialog.open && Array.from(document.querySelectorAll('dialog[open]')).at(-1) === dialog) {
        event.preventDefault();
        if (canEdit)
            void save();
    } };
    window.addEventListener('keydown', onKey);
    const cleanup = () => { closed = true; clearTimeout(previewTimer); previewAbort?.abort(); resizeObserver?.disconnect(); builder?.destroy(); window.removeEventListener('beforeunload', beforeUnload); window.removeEventListener('keydown', onKey); window.removeEventListener('message', onMessage); dialog.removeEventListener('close', cleanup); dialog.removeEventListener('studio-replace', cleanup); };
    dialog.addEventListener('close', cleanup);
    dialog.addEventListener('studio-replace', cleanup);
    async function save(action = 'save') {
        if (busy || !canEdit)
            return;
        form.querySelectorAll(':invalid').forEach(el => { let parent = el.parentElement; while (parent && parent !== form) {
        ...
        error.textContent = '';
        form.inert = true;
        dialog.querySelectorAll('.editor-footer button').forEach(b => b.disabled = true);
        try {
            current = current ? await api(`/api/admin/content/${current.id}`, json('PATCH', { data: next, expected_revision: current.revision })) : await api('/api/admin/content', json('POST', { kind, data: next }));
            dirty = false;
            dialog.querySelector('.save-dot').classList.remove('is-dirty');
            updateStatus();
            if (action !== 'save')
                current = await api(`/api/admin/content/${current.id}/${action}`, json('POST', { expected_revision: current.revision }));
            updateStatus();
            dialog.querySelector('#save-status').textContent = `Gespeichert · ${new Date().toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })}`;
            ...
            busy = false;
            form.inert = false;
            dialog.querySelectorAll('.editor-footer button').forEach(b => b.disabled = false);
            updateStatus();
            refresh();
        }
    }
    ...
    dialog.querySelector('#publish-button')?.addEventListener('click', () => { if (confirm('Diesen Stand jetzt öffentlich veröffentlichen? Prüfe vorher Texte, Links und die mobile Vorschau.'))
        void save('publish'); });
    dialog.querySelector('#approve-button')?.addEventListener('click', async () => { if (!current || busy || !confirm('Diesen eingereichten Stand öffentlich freigeben?'))
        return; busy = true; try {
        current = await api(`/api/admin/content/${current.id}/publish`, json('POST', { expected_revision: current.revision }));
        updateStatus();
        toast('Inhalt freigegeben.');
        refresh();
    }
    catch (err) {
        toast(errorMessage(err), true);
    ...
    dialog.querySelector('#unpublish-button')?.addEventListener('click', async () => { if (!current || busy || !confirm('Diesen Inhalt offline nehmen? Ungespeicherte Eingaben werden verworfen; gespeicherte Versionen bleiben erhalten.'))
        return; busy = true; try {
        await api(`/api/admin/content/${current.id}/unpublish`, json('POST', { expected_revision: current.revision }));
        dirty = false;
        dialog.close();
        refresh();
    ...
    dialog.querySelector('#history-button').addEventListener('click', async () => {
        if (!current)
            return;
        try {
            const revisions = await api(`/api/admin/content/${current.id}/history`);
            const panel = dialog.querySelector('#history-panel');
            panel.hidden = !panel.hidden;
            panel.innerHTML = `<div class="panel-heading"><h3>Gespeicherte Versionen</h3>${helpButton('history')}</div><p class="field-hint">Wiederherstellen erzeugt einen neuen Entwurf. Der Live-Stand bleibt unverändert.</p>` + revisions.map(rev => `<div class="history-row"><span><strong>Version ${rev.revision}</strong><small>${date(rev.created_at)} · ${e(rev.name || 'Demo-System')} · ${e(rev.action)}</small></span>${canEdit ? `<button type="button" class="button small" data-restore="${rev.revision}">Wiederherstellen</button>` : ''}</div>`).join('');
            panel.scrollIntoView({ block: 'nearest' });
            panel.querySelectorAll('[data-restore]').forEach(button => button.addEventListener('click', async () => { if (!current || busy || !confirm('Diese Version als neuen Entwurf übernehmen? Ungespeicherte Eingaben gehen verloren.'))
                return; busy = true; try {
                const restored = await api(`/api/admin/content/${current.id}/restore/${button.dataset.restore}`, json('POST', { expected_revision: current.revision }));
                dirty = false;
                await openEditor(restored, kind, user, refresh);
                refresh();
                toast('Version als Entwurf wiederhergestellt.');
            }
            catch (err) {
                toast(errorMessage(err), true);
            }
