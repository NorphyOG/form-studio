...
export const kindName = { page: 'Seite', service: 'Leistung', project: 'Projekt', settings: 'Einstellungen', article: 'Beitrag', campaign: 'Kampagne' };
export const stateName = { draft: 'Entwurf', pending: 'Zur Prüfung', published: 'Veröffentlicht', rejected: 'Überarbeiten' };
export function badge(state) { return `<span class="badge badge-${escape(state)}">${escape(stateName[state] || state)}</span>`; }
export function date(time) { return new Date(time * 1000).toLocaleDateString('de-DE', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }); }
...
export function errorMessage(error) { return error instanceof Error ? error.message : 'Ein unerwarteter Fehler ist aufgetreten.'; }
export function modal(html) {
    const dialog = document.querySelector('#modal');
    if (dialog.open)
        dialog.dispatchEvent(new Event('studio-replace'));
...
let fieldCount = 0;
export function field(label, name, value = '', type = 'text', extra = '', helpKey = name) {
...
export function textarea(label, name, value = '', rows = 3, extra = '', helpKey = name) {
    const id = `field-${++fieldCount}`;
...
export function select(label, name, value, items, extra = '', helpKey = name) {
    const id = `field-${++fieldCount}`;
...
export function check(label, name, value, helpKey = name, extra = '') {
    const id = `field-${++fieldCount}`;
...
export function overlay(html, className = '') {
    ...
    if (heading) {
        heading.id = `dialog-title-${++fieldCount}`;
        dialog.setAttribute('aria-labelledby', heading.id);
    }
    else
        dialog.setAttribute('aria-label', 'Zusätzliche Auswahl');
    document.body.append(dialog);
    dialog.showModal();
    dialog.querySelectorAll('[data-close]').forEach(b => b.addEventListener('click', () => dialog.close()));
    dialog.addEventListener('close', () => { dialog.remove(); if (previous?.isConnected)
        previous.focus(); }, { once: true });
    return dialog;
}
export function publicPath(record) {
    const slug = record.live?.slug || record.data.slug;
    if (record.kind === 'article')
        return '/news/' + slug;
    if (record.kind === 'campaign')
        return '/admin/preview/' + record.id;
    if (record.kind === 'page')
        return slug === 'home' ? '/' : '/p/' + slug;
    return (record.kind === 'service' ? '/leistungen/' : '/arbeiten/') + slug;
}
export function bindAsync(element, event, callback) {
...
export function icon(name) { return `<svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${symbols[name] || symbols.overview}</svg>`; }
