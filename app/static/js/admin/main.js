...
async function authenticate() {
    ...
    try {
        user = await api('/api/admin/session');
        setCSRF(user.csrf);
        shell();
    }
    catch {
...
function renderLogin(setup) {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            await api(setup ? '/api/setup' : '/api/login', json('POST', data));
            await authenticate();
        }
        catch (err) {
            app.querySelector('#auth-error').textContent = errorMessage(err);
        }
        finally {
...
function shell() {
    ...
    if (user.role !== 'editor') {
        groups[2][1].push(['inbox', 'Anfragen']);
        groups[0][1].push(['analytics', 'Analyse']);
    }
    if (user.role === 'admin')
        groups.push(['VERWALTUNG', [['team', 'Team & Rollen'], ['settings', 'Einstellungen'], ['activity', 'Aktivität']]]);
...
async function renderRoute() {
    ...
    app.querySelectorAll('[data-nav]').forEach(link => { link.classList.toggle('active', link.dataset.nav === route); if (link.dataset.nav === route) {
        link.setAttribute('aria-current', 'page');
        app.querySelector('#breadcrumb-title').textContent = link.querySelector('span').textContent;
    }
    else
    ...
    try {
        ...
        switch (route) {
            case 'pages':
                await contentView(buffer, 'page', u, refresh);
                break;
            case 'services':
                await contentView(buffer, 'service', u, refresh);
                break;
            case 'help':
                helpView(buffer);
                break;
            case 'analytics':
                ...
                await analyticsView(buffer, u);
                break;
            case 'articles':
                await contentView(buffer, 'article', u, refresh);
                break;
            case 'campaigns':
                await contentView(buffer, 'campaign', u, refresh);
                break;
            case 'projects':
                await contentView(buffer, 'project', u, refresh);
                break;
            case 'settings':
                if (u.role !== 'admin')
                    throw new Error('Keine Berechtigung.');
                await contentView(buffer, 'settings', u, refresh);
                break;
            case 'moderation':
                await moderationView(buffer, u, refresh);
                break;
            case 'inbox':
                await inboxView(buffer, u, refresh);
                break;
            case 'media':
                await mediaView(buffer, u, refresh);
                break;
            case 'library':
                await libraryView(buffer, u, refresh);
                break;
            case 'team':
                await teamView(buffer, u, refresh);
                break;
            case 'activity':
                await activityView(buffer);
                break;
            default: await overviewView(buffer, u, refresh);
        }
        if (tick === rendering) {
            view.replaceChildren(buffer);
        }
    }
    catch (err) {
        if (tick === rendering) {
            view.innerHTML = `<div class="panel empty-state"><h2>Die Ansicht konnte nicht geladen werden.</h2><p>${e(errorMessage(err))}</p><button class="button" id="retry-view">Erneut versuchen</button></div>`;
            view.querySelector('#retry-view')?.addEventListener('click', () => void renderRoute());
        }
    }
}
window.addEventListener('hashchange', () => void renderRoute());
window.addEventListener('session-expired', () => { if (user) {
    user = null;
    renderLogin(false);
    toast('Sitzung abgelaufen. Bitte neu anmelden.', true);
} });
