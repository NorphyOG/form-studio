...
export async function analyticsView(container, user) {
    let days = 30, tick = 0;
    ...
    <div class="analytics-footnote"><p>Auswertung: ${data.retention_days} Tage. Ältere Tageswerte werden beim nächsten gezählten Ereignis bereinigt · Tagesgrenzen: UTC · Aktive Tabs: Arbeitsspeicher einer Serverinstanz. Keine IP-Adressen, Geräteprofile, Referrer oder vollständigen URLs in der Statistik gespeichert.</p>${user.role === 'admin' ? '<button class="button small danger-text" type="button" data-erase>Statistikdaten löschen</button>' : ''}</div>`;
            ...
            body.querySelector('[data-erase]')?.addEventListener('click', async () => { if (!confirm('Alle Statistik-Aggregate aus der aktiven Datenbank löschen? Bestehende Backups bleiben unverändert.'))
                return; try {
            ...
            body.textContent = errorMessage(err);
        }
    }
    container.querySelectorAll('[data-days]').forEach(button => button.addEventListener('click', () => { days = Number(button.dataset.days); container.querySelectorAll('[data-days]').forEach(b => b.setAttribute('aria-pressed', String(b === button))); void load(); }));
    ...
    const interval = window.setInterval(() => { if (!container.isConnected) {
        clearInterval(interval);
        return;
    } if (!document.hidden && !body.contains(document.activeElement))
        void load(); }, 30000);
}
