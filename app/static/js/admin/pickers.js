...
export async function pickMedia(onPick) {
    ...
    async function load() {
        const tick = ++sequence;
        try {
            const items = await api('/api/admin/media?' + new URLSearchParams({ q: search.value, limit: '24', offset: String(offset) }));
            if (tick !== sequence || !dialog.open)
                return;
            ...
            dialog.querySelectorAll('[data-media]').forEach(b => b.addEventListener('click', () => { onPick(items.find(m => m.id === b.dataset.media)); dialog.close(); }));
            ...
            if (dialog.open)
                dialog.querySelector('.picker-results').textContent = errorMessage(err);
        }
    }
...
export async function pickCampaign(onPick) {
    ...
    async function load() {
        const tick = ++sequence;
        try {
            const result = await api('/api/admin/content-index?' + new URLSearchParams({ kind: 'campaign', q: search.value, limit: '24', offset: String(offset) }));
            if (tick !== sequence || !dialog.open)
                return;
            ...
            dialog.querySelectorAll('[data-campaign]').forEach(b => b.addEventListener('click', () => { onPick(result.items.find(i => i.id === b.dataset.campaign)); dialog.close(); }));
            dialog.querySelector('[data-prev]').addEventListener('click', () => { offset = Math.max(0, offset - 24); void load(); });
            dialog.querySelector('[data-next]').addEventListener('click', () => { offset += 24; void load(); });
        }
        catch (err) {
...
export function setMediaSelect(select, media) {
    if (media && !Array.from(select.options).some(o => o.value === media.id))
        select.add(new Option(media.alt, media.id));
